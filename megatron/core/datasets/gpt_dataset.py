...
class GPTDatasetConfig(BlendedMegatronDatasetConfig):
    ...
    def __post_init__(self) -> None:
        ...
        if self.goldfish_loss:
            assert isinstance(self.goldfish_k, int), "goldfish frequency must be an integer."
            assert isinstance(self.goldfish_h, int), "goldfish context width must be an integer."   
...
class GPTDataset(MegatronDataset):
    ...
    def __getitem__(self, idx: Optional[int]) -> Dict[str, torch.Tensor]:
        ...
        if self.config.add_extra_token_to_sequence: # Here
            tokens = text[:-1].contiguous() # Just 1d 
...
def _get_ltor_masks_and_position_ids(
    ...
    """
    seq_length = data.numel() # decimal, total number of elements in data
    ...
    # Loss mask.
    loss_mask = torch.ones(seq_length, dtype=torch.float, device=data.device) # (seq_length, )
    if eod_mask_loss:
